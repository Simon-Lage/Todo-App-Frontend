...
const ProjectImagesPage: React.FC = () => {
  ...
  const canUpload = Boolean(authSession.permissions?.['perm_can_edit_projects']);
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setUploading(true);
      setError(null);
      await imageService.uploadForProject(projectId, file);
      await loadImages();
    } catch (err) {
  ...
  return (
    <IonContent className="app-page-content">
      <div className="page-header">
        <h1 className="page-title">Projekt-Bilder</h1>
      ...
      <div style={{ padding: '16px' }}>
        <IonButton routerLink={`/app/project/${projectId}`} expand="block" fill="outline" className="app-button-secondary">
